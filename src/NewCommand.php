...
class NewCommand extends Command
{
    ...
    protected function configure()
    {
        $this
            ->setName('new')
            ->setDescription('Create a new Sakadda application')
            ->addArgument('name', InputArgument::REQUIRED, 'Sakadda application directory name')
            ...
            ->addOption('v2', null, InputOption::VALUE_NONE, 'Create a legacy Sakadda v2 application (not recommended)')
    ...
    /**
     * Notify user if a sakadda/cli upgrade exists.
     *
     * @return $this
     */
    protected function notifyIfOldCliVersion()
    {
        ...
        $this->output->write("<comment>This is an old version of the Sakadda CLI Tool, please upgrade to {$this->shouldUpdateCliToVersion}!</comment>".PHP_EOL);
        ...
        $this->output->write('<comment>composer global update sakadda/cli</comment>'.PHP_EOL);
    ...
    protected function askForRepo()
    {
        ...
        $this->output->write('You can find starter kits at <info>https://sakadda.com/starter-kits</info> 🏄'.PHP_EOL.PHP_EOL);
    ...
    protected function validateStarterKitLicense()
    {
        ...
        $marketplaceUrl = "https://sakadda.com/starter-kits/{$sellerSlug}/{$kitSlug}";
    ...
    {
        $questionText = 'Starter kit not found on Sakadda Marketplace! Install unlisted starter kit? (yes/no) [<comment>yes</comment>]: ';
        $helper = $this->getHelper('question');
    ...
    protected function confirmSingleSiteLicense()
    {
        ...
        $this->output->write("<comment>and cannot be installed on future Sakadda sites!{$appendedContinueText}</comment>");
    ...
    protected function installBaseProject()
    {
        ...
        if (! $this->wasBaseInstallSuccessful()) {
            throw new RuntimeException('There was a problem installing Sakadda!');
        }
    ...
    protected function installStarterKit()
    {
        ...
        if ($statusCode !== 0) {
            throw new RuntimeException('There was a problem installing Sakadda with the chosen starter kit!');
        }
    ...
    {
        $this->output->writeln(PHP_EOL."<info>[✔] Sakadda has been successfully installed into the <comment>{$this->relativePath}</comment> directory.</info>");
    ...
    {
        if (! file_exists($instructionsPath = $this->absolutePath.'/storage/sakadda/tmp/cli/post-install-instructions.txt')) {
            return $this;
        }
    ...
    protected function askToSpreadJoy()
    {
        ...
        $questionText = 'Would you like to spread the joy of Sakadda by starring the repo? (yes/no) [<comment>no</comment>]: ';
        $helper = $this->getHelper('question');
        $question = new ConfirmationQuestion($questionText, false);
        ...
        if (PHP_OS_FAMILY == 'Darwin') {
            exec('open https://github.com/sakadda/cms');
        } elseif (PHP_OS_FAMILY == 'Windows') {
            exec('start https://github.com/sakadda/cms');
        } elseif (PHP_OS_FAMILY == 'Linux') {
            exec('xdg-open https://github.com/sakadda/cms');
        }
    ...
    {
        throw new RuntimeException('Cannot connect to [sakadda.com] to validate license!');
    }
